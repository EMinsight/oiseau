...
std::pair<xt::xarray<T>, xt::xarray<T>> jacobi_gq(unsigned n, Real alpha, Real beta) {
  ...
  Real cte = std::pow(2, (alpha + beta + 1)) / (alpha + beta + 1) * tgamma(alpha + 1) *
...
xt::xarray<T> vandermonde_1d(unsigned n, const xt::xarray<T> &r) {
...
template <std::floating_point T = double>
xt::xarray<T> grad_vandermonde_1d(unsigned n, const xt::xarray<T> &r) {
  std::array<size_t, 2> shape = {r.size(), n + 1};
  xt::xtensor<double, 2> output(shape);
  for (size_t i = 0; i < n + 1; ++i) {
