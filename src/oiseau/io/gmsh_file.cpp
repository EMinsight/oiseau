...
EntitiesSection entities_handler(std::istream& f_handler, bool is_binary) {
  auto quantity = from_file<4, std::size_t>(f_handler, is_binary);
  ...
  for (int d = 0; d < 4; d++) {
    for (int j = 0; j < quantity[d]; j++) {
      auto [tag] = from_file<1, int>(f_handler, is_binary);
      auto bounding_coods = from_file<double>(f_handler, (d == 0) ? 3 : 6, is_binary);
      auto [num_physicals] = from_file<1, std::size_t>(f_handler, is_binary);
      auto physical_tags = from_file<int>(f_handler, num_physicals, is_binary);
      if (d > 0) {
        auto [num_BREP] = from_file<1, std::size_t>(f_handler, is_binary);
...
NodesSection nodes_handler(std::istream& f_handler, bool is_binary) {
  auto [num_entity_blocks, total_num_nodes, min_node_tag, max_node_tag] =
      from_file<4, std::size_t>(f_handler, is_binary);

  std::vector<NodesBlock> blocks;
  blocks.reserve(num_entity_blocks);

  for (int i = 0; i < num_entity_blocks; i++) {
    auto [dim, entity_tag, parametric] = from_file<3, int>(f_handler, is_binary);
    auto [quantity] = from_file<1, std::size_t>(f_handler, is_binary);
    ...
    for (int j = 0; j < quantity; j++) {
      auto [tag] = from_file<1, std::size_t>(f_handler, is_binary);
      node_tags.emplace_back(tag);
    }
    for (int j = 0; j < quantity; j++) {
      auto xyz = from_file<3, double>(f_handler, is_binary);
...
ElementSection elements_handler(std::istream& f_handler, bool is_binary) {
  ...
  for (std::size_t i = 0; i < num_element_blocks; i++) {
    auto [entity_dim, entity_tag, element_type] = from_file<3, int>(f_handler, is_binary);
    auto [num_elements_in_block] = from_file<1, std::size_t>(f_handler, is_binary);
